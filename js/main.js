...
// Una promesa es una funcion asincrona
...
//                             }
//             setTimeout( ()=>{ // esto es para simular
//                 resolve (producto);
//                 }, 2000);
//             }); 
//                         };
...
function getProducto(){
    ...
    promesa.then((response) => {
        response.json().then((prods)=>{  // el .jsaon es otra promesa, por eso necesita un then
    ...
    }); // este es el catch de promesa
}
        
getProducto();

//     .then((prod)=>catalogoProductos(prod))   //resolve
//     .catch((err) => console.log(err.message));     //reject
        
function catalogoProductos(prods){
    let catalogo = "";
    prods.forEach(function(imagen){
                catalogo += `
                            <div class="card" style="width: 18rem; margin-bottom:2rem;">
                            <img src="${imagen.image}" class="card-img-top" style: "max-width=12rem; height: 16rem;" +  alt="${imagen.id}">
                            <div class="card-body">
                            <h5 class="card-title">${imagen.title}</h5>
                            <p class="card-text">${imagen.description}</p>
                            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal"_${imagen.id}>More info</button>
                            </div>
                            </div>
                            <!-- Termina Class -->
                            <!-- Modal -->
                            <div class="modal fade" id="exampleModal"_${imagen.id} tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    productos.insertAdjacentHTML("beforeend",catalogo);
                                    };